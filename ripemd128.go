...
func _Block128(md *ripemd128digest, p []byte) int {
	n := 0
	var x [16]uint32
	var alpha uint32
	for len(p) >= BlockSize128 {
		...
		for i < 32 {
			...
			// parallel line
			alpha = aa + (dd ^ (bb | ^cc)) + x[_128n1[i]] + 0x5c4dd124
			s = int(_128r1[i])
			alpha = bits.RotateLeft32(alpha, s)
			aa, bb, cc, dd = dd, alpha, bb, cc
		...
		// round 3
		for i < 48 {
			alpha = a + (d ^ (b | ^c)) + x[_128n0[i]] + 0x6ed9eba1
			s := int(_128r0[i])
			alpha = bits.RotateLeft32(alpha, s)
			a, b, c, d = d, alpha, b, c
